...
#| label: cleaning
#| message: false
salinity_clean <- salinity |> # use the salinity data
  clean_names() |> # clean the column names 
  select(salinity_m_s_cm, pickleweed) |> # select columns of interest
  rename(salinity = salinity_m_s_cm, # rename salinity
         biomass = pickleweed) # rename biomass 
...
```{r}
#| label: biomass and salinity scatterplot
#| message: false
#| fig-height: 4
#| fig-width: 6
ggplot(data = salinity_clean, # use clean data frame
       aes(x = salinity, # set salinity on x-axis and biomass on y-axis
            y = biomass)) + 
  geom_point(size = 2, # adding scatterplot points
             color = "seagreen", # changing the color
             alpha = 21) + # setting the points to be a circle
  labs(x = "Soil Salinity (mS/cm)", # label the x-axis
       y = "California Pickleweed Biomass (g)", # label the y-axis
       title = "Relationship between Soil Salinity and Pickleweed Biomass") + # add a title 
...
#| message: false
ggplot(data = salinity_clean, # use clean data frame
       aes(sample = salinity)) + # set salinity for QQ plot
  geom_qq_line(color = "seagreen") + # reference line
  geom_qq() + 
  labs(x = "Theoretical Quantiles", # label the x-axis
       y = "Sample Quantiles", # label the y-axis
       title = "QQ Plot of Soil Salinity") + # add a title 
...
#| message: false
ggplot(data = salinity_clean, # use clean data frame
       aes(sample = biomass)) + # set salinity for QQ plot
  geom_qq_line(color = "seagreen") + # reference line
  geom_qq() + 
  labs(x = "QQ Plot of Biomass", # label the x-axis
       y = "Sample Quantiles", # label the y-axis
       title = "QQ Plot of Soil Salinity") + # add a title 
  theme_light() # apply light theme
...
#| label: pearson test
#| message: false
cor.test(salinity_clean$salinity, salinity_clean$biomass, # test relationship between salinity and biomass
...
clean_names() |> # clean the data names
  ...
  mutate(hw_mins = hw*60) # adding a column that makes the hours into mins
...
ggplot(data = my_data_clean, # start with the my_data_clean data frame
       aes(x = with_someone, # categorical predictor variable on the x-axis
           ...
           fill = with_someone)) + # color by category
  geom_boxplot(alpha = 0.6, # first layer should be a boxplot
               width = 0.5,
               outlier.shape = NA) + 
  geom_jitter(width = 0.2, # making the points jitter horizontally
              height = 0,
              alpha = 0.5, # apply transparency
              size = 2, # adjust point size
              color = "black") + 
  stat_summary(fun = mean, # show mean point
               geom = "point",
               shape = 23, # set shape to be a diamond 
               size = 3, # adjust point size
               fill = "white") + # set color of this mean point 
  scale_fill_manual(values = c("No" = "orangered3", # set custom colors
                               "Yes" = "limegreen")) +
  ...
  theme_minimal(base_size = 12) + # apply minimal theme
  theme(legend.position = "none", # remove legend and adjust styling
...
ggplot(my_data_clean, # start with the my_data_clean data frame
       aes(x = hw_mins, # categorical variable on x-axis
           y = min, # devotional minutes on y-axis
           shape = location, # shape by category
           fill = location)) + # color by category
  geom_point(size = 4, # adjust point size
             alpha = 0.8, # apply transparency
             color = "black") + 
  scale_shape_manual(values = c(21, 22, 23, 24, 25)) + # set point shapes
  scale_fill_viridis_d(option = "G") + # apply viridis color palette
  coord_cartesian(ylim = c(0, 130)) + # limit y-axis range
  labs(x = "Time spent doing Homework/Classes (mins)", # label x- and y- axes, add a title and subtitle
       y = "Bible Study Devotional (mins)",
       title = "Daily Homework Time vs. Bible Study",
       subtitle = "Most recent observation: 25 Feb 2026") +
  theme_light() + # apply light theme
  theme(legend.position = "right", # set position of legend and adjust styling 
        plot.title = element_text(face = "bold"))
...
The work is presented as a scrapbook-style collage, combining illustrated elements such as hills, sheep, and symbolic imagery with layered textures and a cut-out graphic look. The scrapbook aesthetic allows the visualization to feel personal and reflective, similar to documenting moments in a journal.

##### • Process

I began by tracking my daily data on Bible study and academic workload. Then I designed visual symbols for each variable and assembled them into a digital countryside scene. I want to do a scrapbook collage approach-laying textures, shapes, and drawings to represent my data.
